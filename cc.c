...
G swap(G*x,G*y){G t=*x;*x=*y;*y=t;R t;}    // store load  immediate
I as(G*b,G o,G x,G y,G m,J z) //m is 1:RR, 2:MR, 4:RM, 8:RI.  return inst length
{G*a=b;
 G rex(G w){R 0x48|w*8|(x>7)*4|y>7;}
 G modrm(){R (0||m|6)*0xC0|(x&7)*8|y&7;}
 G*dir(G p){if(m&2)swap(&x,&y),p+=2;*a++=rex(0),*a++=p,*a++=modrm();}
 switch(o){
 C(ret,*a++=0xC3)
 C(add,dir(0x01))
 C(sub,dir(0x29))
 C(mov,(m&8?*a++=0x48+(x>7),*a++=0xb8+x%8,*(J*)a=z,a+=8  //immediate, 
           :dir(0x89)))
 C(cqo, (*a++=0x48,*a++=0x99))
